...
```{r, echo=FALSE}
doorae$regi.kr <- factor(doorae$"사업자등록여부", 
                         levels=c("Yes", "No"), 
                         labels=c("등록", "미등록"))
doorae$busi.kr <- factor(doorae$"성과_사업착수", 
                         levels=c("Yes", "No"), labels=c("착수", "미착수"))
...
```{r, echo=FALSE}
comm_stat.org_str.reg.ftbl <- ftable(table("공동체상태 :" = doorae$"공동체상태",
                                           "조직형태 :" = doorae$"조직형태",
                                           "사업자등록여부:" = doorae$regi.kr))
comm_stat.org_str.busi.ftbl <- ftable(table("공동체상태 :"=doorae$"공동체상태",
                                            "조직형태 :" = doorae$"조직형태",
                                            "사업착수여부 :" = doorae$busi.kr))
```

```{r, echo=FALSE, results='asis'}
comm_stat.org_str.reg.ptbl <- prop.table(comm_stat.org_str.reg.ftbl, margin = 1) * 100
kable(as.matrix(round(comm_stat.org_str.reg.ptbl, digits=1)), 
      caption = "* 공동체상태/조직형태와 사업자등록여부(%)")
```

```{r, echo=FALSE, results='asis'}
kable(as.matrix(comm_stat.org_str.busi.ftbl), 
      caption = "* 공동체상태/조직형태와 사업착수여부")
```

```{r, echo=FALSE, results='asis'}
comm_stat.org_str.busi.ptbl <- prop.table(comm_stat.org_str.busi.ftbl, margin = 1) * 100
kable(as.matrix(round(comm_stat.org_str.busi.ptbl, digits = 1)), 
      caption = "* 공동체상태/조직형태와 사업착수여부(%)")
```
...
```{r, results='asis', echo=FALSE}
status.vs.corporate_type <- table(doorae$"공동체상태", doorae$"법인유형", 
                                  dnn = c("공동체상태", "조직유형"))
kable(status.vs.corporate_type, 
      caption = "<공동체상태와 조직유형>")
```

```{r, results='asis', echo=FALSE}
stock.co <- which(doorae$"법인유형"=="주식회사")
kable(doorae[stock.co, c("공동체상태", "조직명")], 
      caption = "<주식회사인 공동체사업체>")
```
...
kable(status.vs.business_item.am, 
      caption = "<공동체상태와 사업아이템>")
```
...
kable(round(status.vs.business_item.p.am, digits = 1), 
      caption = "<공동체상태와 사업아이템(%)>")
```
...
box.1 <- ggplot(doorae.ys, aes(x = status, y = years)) + 
  theme_bw() + 
  geom_boxplot() + 
  theme.kr.Malgun
box.2 <- box.1 + 
...
box.4 <- box.3 + 
  annotate("text", x = c(1, 2), y = c(12.2, 13), 
...
library(ggplot2)
ecdf.1 <- ggplot(data = doorae.ys, mapping = aes(x = years, colour = status)) 
...
ecdf.11 <- ecdf.10 + annotate("segment", x = 1.37, xend = 4.46, y = 0.5, yend = 0.5,
                              colour = "blue", 
                              arrow = arrow(ends="both", length = unit(0.2, "cm")))
ecdf.12 <- ecdf.11 + scale_x_continuous(breaks = c(0, 1.3, 4.39, 12.3), 
                                        labels = c(0, 1.3, 4.39,  12.3))
ecdf.13 <- ecdf.12 + annotate("text", x = 2.8, y = 0.54, 
                              label = "중위수의 차이", 
                              size = 3, 
                              family="Malgun Gothic")
ecdf.14 <- ecdf.13 + annotate("text", x = 2.8, y = 0.46, 
                              label = "약 3년", 
                              size = 3, 
                              family="Malgun Gothic")
ecdf.14
...
poly.11 <- poly.10 + scale_x_continuous(breaks = c(0, 1.3, 4.39, 12.3), 
                                        labels=c(0, 1.3, 4.39, 12.3))
poly.12 <- poly.11 + annotate("text", x = 4.4, y = 0.78, 
                              label = "두 도표 간 면적은\n평균의 차이,\n약 3년", 
                              size = 4, 
                              angle = 15, 
                              family="Malgun Gothic")
poly.13 <- poly.12 + annotate("text", x = 6, y = 0.6, 
                              label = "경영개선", 
                              size = 4, 
                              family = "Malgun Gothic")
poly.14 <- poly.13 + annotate("text", x = 1, y = 0.9, 
                              label = "창업준비", 
                              size = 4, 
                              family="Malgun Gothic")
poly.14
...
d.1 <- ggplot(doorae, aes(x = 업력, y = name)) + 
...
d.3 <- d.2 + 
  scale_colour_brewer(palette = "Set1", 
                      limits = c("경영개선", "창업준비"))
d.4 <- d.3 + theme_bw() + theme.kr.Malgun
d.6 <- d.4 + ylab("조직명") 
d.7 <- d.6 + 
