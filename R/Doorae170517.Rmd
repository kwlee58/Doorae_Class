...
box.1 <- ggplot(doorae.ys, aes(x = status, y = years)) + geom_boxplot() + theme_bw() + theme.kr.HCR
...
ecdf.3 <- ecdf.2 + theme_bw() + theme.kr.HCR
...
ecdf.13 <- ecdf.12 + annotate("text", x = 4.5, y = 0.54, label = "중위수의 차이", size = 3, family = "HCR Dotum LVT")
ecdf.14 <- ecdf.13 + annotate("text", x = 4.5, y = 0.46, label = "약 3년", size = 3, family = "HCR Dotum LVT")
ecdf.14
```
...
poly.3 <- poly.2 + theme_bw() + theme.kr.HCR
...
poly.12 <- poly.11 + annotate("text", x=6, y=0.75, label="두 도표 간 면적은\n평균의 차이,\n약 3년", size=4, angle=15, family="HCR Dotum LVT")
poly.13 <- poly.12 + annotate("text", x=9, y=0.6, label="경영개선", size=4, family="HCR Dotum LVT")
poly.14 <- poly.13 + annotate("text", x=2, y=0.9, label="창업준비", size=4, family="HCR Dotum LVT")
poly.14
```
