...
```{r, echo=FALSE, results='asis'}
kable(as.matrix(comm_stat.org_str.reg.ftbl), caption = "* 공동체상태/조직형태와 사업자등록여부 ")
```
...
pander(status.vs.corporate_type, caption="<공동체상태와 조직유형>")
````

```{r, results='asis', echo=FALSE}
stock.co<-which(doorae$"법인유형"=="주식회사")
# kable(doorae[stock.co, c("공동체상태", "조직명")], caption="<주식회사인 공동체사업체>")
pander(doorae[stock.co, c("공동체상태", "조직명")], caption="<주식회사인 공동체사업체>")
```
...
box.1 <- ggplot(doorae.ys, aes(x=status, y=years)) + theme_bw() + geom_boxplot() + theme.kr.ASGN 
box.2 <- box.1 + ggtitle("공동체상태와 업력") + xlab("공동체상태") + ylab("업력(년)") 
box.3 <- box.2 + scale_x_discrete(labels=c("창업준비", "경영개선")) 
box.4 <- box.3 + annotate("text", x=c(1, 2), y=c(12.2, 13), label=c("예그리나", "아름다운인제관광"), size=3.5, family="AppleGothic")
box.4
```
...
ecdf.3 <- ecdf.2 + theme_bw() 
# ecdf.4 <- ecdf.3 + theme(panel.background=element_rect("white"), panel.border=element_rect(colour="black", fill=NA))
ecdf.5 <- ecdf.3 + ggtitle("공동체상태와 업력") + theme(text=element_text(family="Apple SD Gothic Neo"))
...
ecdf.9 <- ecdf.8 + scale_colour_discrete(labels=c("창업준비", "경영개선")) + theme(legend.text=element_text(family="Apple SD Gothic Neo"))
ecdf.10 <- ecdf.9 + theme(legend.position=c(0.8, 0.5))
ecdf.11 <- ecdf.10 + annotate("segment", x=1.37, xend=4.46, y=0.5, yend=0.5, colour="blue", arrow=arrow(ends="both", length=unit(0.2, "cm")))
...
poly.3 <- poly.2 + theme_bw()
poly.4 <- poly.3 + theme(panel.background=element_rect("white"), panel.border=element_rect(colour="black", fill=NA))
poly.5 <- poly.4 + ggtitle("공동체상태와 업력") + theme(text=element_text(family="Apple SD Gothic Neo"))
...
poly.8 <- poly.7 + labs(colour="공동체상태") + theme(legend.title=element_text(family="Apple SD Gothic Neo"))
poly.9 <- poly.8 + scale_colour_discrete(labels=c("창업준비", "경영개선")) + theme(legend.text=element_text(family="Apple SD Gothic Neo"))
...
poly.12 <- poly.11 + annotate("text", x=4.4, y=0.78, label="두 도표 간 면적은\n평균의 차이,\n약 3년", size=4, angle=15, family="Apple SD Gothic Neo")
poly.13 <- poly.12 + annotate("text", x=6, y=0.6, label="경영개선", size=4, family="Apple SD Gothic Neo")
poly.14 <- poly.13 + annotate("text", x=1, y=0.9, label="창업준비", size=4, family="Apple SD Gothic Neo")
poly.14
```
...
d.6 <- d.5 + ylab("조직명") + theme(axis.title.y=element_text(family="Apple SD Gothic Neo")) 
d.7 <- d.6 + theme(axis.text.y=element_text(family="Apple SD Gothic Neo", size=6)) 
d.8 <- d.7 + theme(panel.grid.major.y=element_blank(), legend.position=c(1, 0.55), 
